...
本项目从 [wangwangit/nav](https://github.com/wangwangit/nav)（拾光集）迭代而来。保留了上游的基础骨架（Cloudflare Workers + D1 + KV、后台 CRUD、访客投稿、HttpOnly Cookie 会话），在此之上做了架构改造与大量功能增强：

| 维度 | 上游（拾光集） | 本项目 |
|---|---|---|
| 架构 | 单文件 Worker + HTML 模板拼接 | 模块化分层（handlers / pages / services / lib），60+ 模块 |
| 部署 | wrangler 手动部署 | GitHub Actions 自动部署 + schema.sql 幂等初始化 + 质量门禁（79 个自动化测试、语法检查） |
| 分类 | 扁平分类、自定义排序 | **父子分类**（层级树、前台展开/收起、图标/颜色/描述元数据） |
| 搜索 | 站内模糊搜索 | 全站高级搜索：`tag:` / `cat:` / `url:` / `is:` 语法、中文 n-gram、首字母召回、可解释排序 |
| 前台 | 单布局 | 卡片 / 列表 / 分组 / 瀑布流 / 概览 5 种布局 + 首屏分页渲染 |
| 认证 | 管理员 Cookie 会话（12h 滑动） | + 会话绝对过期（7 天上限）、PBKDF2 密码哈希、登录/锁/私密三套试错限速 |
| 访问控制 | 无 | **整站锁**（部署级门禁、解锁时长多档可选）、**私人书签密码**、四档可见性（public / private / unlisted / admin_only） |
| 标签 | 无 | 标签系统 + 前台按标签筛选 |
| AI | 无 | **AI 书签助理**（RAG 优先检索本站书签）、AI 推荐分类/标签、模型思考开关 |
| 健康检测 | 无 | 单条/批量链接可用性检测 + Cron 定时巡检 |
| 数据管理 | 导入/导出 | + **备份恢复**（手动/定时、30 份滚动保留、覆盖/合并、恢复前快照） |
| API | 内部接口 | 公开 API + API Discovery + OpenAPI 描述 + **Bearer Token 分级**（read / write / admin） |
| 浏览器插件 | 无 | MV3 插件：一键收藏（查重、强制保存、自动抓取、AI 推荐）、**站内书签浏览**、**一键同步** |
| 同步 | 无 | **以浏览器收藏为事实源的全量对齐同步**（新增/改名/换分类/删除跟随浏览器，手动书签永不覆盖，支持 bookmarks.html 上传） |
| WebHook | 无 | 写操作事件分发 + HMAC 签名校验 |
| 通知 | 无 | 系统公告（Markdown、仅显示一次） |
| 运维 | 无 | 操作日志、提交分析（趋势/热力图/分类占比）、部署清单 / API 指南 / ADR 文档 |
| 性能 | 边缘缓存 | + 首页匿名边缘缓存、运行时迁移 KV 门控、KV 续期降频、站点索引延迟加载、锁状态 KV 缓存 |

> 功能对比基于上游仓库公开形态（README 与代码结构）整理；本项目实现细节以本仓库代码为准。沿用上游 MIT 许可证，详见 [LICENSE](LICENSE)。
