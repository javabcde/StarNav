...
本项目从 [Quanda666/StarNav](https://github.com/Quanda666/StarNav) 迭代而来（其上游为 [wangwangit/nav](https://github.com/wangwangit/nav) 拾光集），在模块化架构基础上持续增强，适合作为个人导航站、团队工具导航站或轻量级书签管理系统长期维护。
...
本项目从 [Quanda666/StarNav](https://github.com/Quanda666/StarNav) 迭代而来（后者基于 [wangwangit/nav](https://github.com/wangwangit/nav) 拾光集）。上游已具备：多布局前台、父子分类、标签、全站高级搜索、私人书签、AI 书签助理、健康检测、备份恢复、开放 API + Bearer Token、MV3 插件收藏、WebHook、提交分析等。本项目在继承这些能力之外，新增与增强了以下部分：

| 维度 | 上游（Quanda666/StarNav） | 本项目 |
|---|---|---|
| 自动部署 | 仅 GitHub Actions 质量检查 | + `deploy.yml` push 自动部署 Worker 并幂等执行 `schema.sql`；`release.yml` 自动发版 |
| 整站锁 | 无 | **部署级访问门禁**：后台设置密码即启用，除白名单路由外全站需解锁；解锁时长 仅本次会话 / 1h / 12h / 7d / 30d；PBKDF2 哈希 + 试错限速 |
| 书签同步 | 无 | **一键同步浏览器收藏**：以浏览器收藏为事实源全量对齐（新增/改名/换分类/删除跟随浏览器），手动书签永不覆盖；插件一键 + 后台 bookmarks.html 上传双入口；解除同步、写操作日志、空快照拦截 |
| 插件 | 单视图收藏 | **三视图弹窗**：站内书签浏览（搜索、父子分类手风琴、无限滚动、分类 chip、跃迁反馈）+ 收藏（查重仅在收藏视图提示）+ 同步；缓存预热 + 过期静默刷新；缓存间隔可选 6h/12h；站点名跟随后台设置；测试连接并行化 + 10s 超时 |
| AI | 助理 + 分类/标签推荐 | + **模型思考开关**（`enable_thinking` 显式控制，推理模型默认关思考） |
| 性能工程 | — | 首页匿名边缘缓存、运行时迁移 KV 门控（冷 isolate 不再跑迁移 batch）、KV 滑动续期降频（每请求 4 写 → 约每 6 小时 1 写）、站点索引延迟加载（`/api/usage-sites` 按需拉取）、整站锁状态 KV 缓存 |
| 工程质量 | 语法检查 + 测试 | + 79 个自动化测试用例（迁移门控 / 可见性 / 缓存短路 / 续期降频等）、部署后线上延迟实测验证 |

> 差异整理基于上游仓库公开 README 与代码结构；本项目实现细节以本仓库代码为准。沿用上游 MIT 许可证，详见 [LICENSE](LICENSE)。
