...
// 站点核心原语（规范行投影 / 可见性谓词 / 载荷规范化 / 去重键 / 重复点查 / 排序前置 / 全量读取）
// 已迁入 siteCore.js（2026-08-16 架构评审候选 1，解环）；本文件 re-export 垫片保持存量测试
// 与调用方 import 面不变（同 ADR-0003 模式）。siteCore 不反向依赖本文件，模块图保持单向。
export { getAllSites, getPrependSortOrder, normalizeSitePayload, buildDuplicateError, normalizeDuplicateUrlKey, findDuplicateSite } from './siteCore.js';
import { SITE_SELECT_COLUMNS, applyVisibilityWhere, getAllSites, getPrependSortOrder, normalizeSitePayload, buildDuplicateError, normalizeDuplicateUrlKey, findDuplicateSite } from './siteCore.js';
