...
// 生成期常量：卡片契约与 themeColors 字面量在模块加载时内插进客户端脚本。
const { wrapperClass, healthBadgeClass, healthBadgeLabel, lastCheckedPrefix } = CARD_CONTRACT;
// 客户端搜索结果卡只带 id/catalog/tags 三项 data 属性（契约属性名的子集）
const [attrId, , , attrCatalog, attrTags] = CARD_CONTRACT.dataAttrs;
// 与迁移前手写的 themeColors 字面量逐字节一致，值来自 accents.js
const themeColorsLiteral = `{${Object.entries(themeColorMap()).map(([name, color]) => `${name}:'${color}'`).join(',')}}`;

export function homeClientScript({ defaultAccent, pageBackgroundImage, defaultLayout, i18n, myUsageScript, frontAdminScript, dragScript, adminAuthed, canDragSort }) {
...
// String.raw 前提守卫（模块加载即执行）：正文不得含反引号；插值必须全部经函数参数
// 收敛——任何残留的 ${ 都意味着插值面漂移，直接抛错，防止输出悄悄偏离。
{
  const probe = homeClientScript({ defaultAccent: 'blue', pageBackgroundImage: false, defaultLayout: 'grid', i18n: undefined, myUsageScript: () => '', frontAdminScript: () => '', dragScript: () => '', adminAuthed: false, canDragSort: false });
  if (probe.includes('`')) throw new Error('clientScript: 正文含反引号，String.raw 输出前提被破坏');
  if (probe.includes('${')) throw new Error('clientScript: 存在未插值的 ${，插值面必须经函数参数收敛');
}
