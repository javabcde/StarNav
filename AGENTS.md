...
| Path | Purpose |
|---|---|
| `src/handlers/` | Request handlers. `api.js` is a table-driven dispatcher (`ROUTES`, see ADR-0004) over `api/resources/*` + `api/spaces.js`; serializers in `api/sites.js`, error handling in `api/errors.js` |
| `src/services/` | D1-backed domain logic. `siteService.js` is the core (query orchestration + CRUD + analytics); pure-logic leaves: `siteCore.js` (shared primitives), `searchScoring.js` (search scoring pipeline), `healthQuery.js` (health predicates), `submissionAnalytics.js`, `webhookPolicy.js`, `aiLocalLogic.js`, `aiModelService.js`; also migration, backup, ai, webhook, sync, settings, private-bookmark |
| `src/lib/` | Infrastructure: `auth.js` (password/ratelimit + shims), `crypto.js` (AES-GCM), `ssrf.js` (safeFetch), `webdav.js` (WebDAV backup adapter), `edgeCache.js`, `i18n.js`, `utils.js` (response factories), `favicon.js`, `apiTokenService.js`, `sessionPolicy.js`, `ipThrottle.js`, `cookie.js` |
| `src/pages/` | Server-rendered HTML via template literals. `home/` fragment renderers, `admin/` render layer (`login.js` login page, `html.js` admin shell, `scripts/`, `styles.js`), `adminAssets.js` (static assets with ETag), `home/generated-css.js` (committed generated CSS) |
| `tests/` | ~50 `*.test.js` files, `node:test`, ~430 tests |
...
| `scripts/` | `build-css.mjs`, `check-syntax.js`, `generate-schema.mjs` (schema.sql ← migrationService 单一源) |
...
npm run schema:generate # migrationService 单一源 → 重新生成 schema.sql（改表结构后必须运行）
...
| `schema.sql` | CI 部署/`db:init` 用的 fresh schema —— **由 `npm run schema:generate` 从 `migrationService.js` 生成，勿手改**（回归锁见 `tests/migrationSchema.test.js`） |
...
- Committed generated artifacts: `src/pages/home/generated-css.js` (deploy depends on it) and `schema.sql` (generated by `npm run schema:generate`); `dist/` is gitignored.
...
- **Coverage gaps** (add tests when touching): `handlers/pwa.js`, `api/sites.js` CRUD beyond the auth gate, `api/spaces.js`, `api/discovery.js`, most pages (behavior tests exist for home render branches and the admin handler), settings services beyond the AI settings domain (its read/write behavior is covered via `aiService.test.js`). Well-covered: site lock, auth/tokens (incl. admin login flow: throttle/session/logout), API errors, bookmark sync, migration (gating + fresh-schema single source), go redirect, favicon, ssrf, edge cache, health tri-state (SQL + JS), search scoring (direct unit matrix), backup lifecycle + WebDAV transport, webhook policy/dispatch, category descendants/color validation, popup logic.
